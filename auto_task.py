...
def connexion_instagram_depuis_selected_user(force_restore=False):
    ...
    if force_restore:
        ...
        try:
            ...
            print(horloge(), color(f"✅ Session restaurée pour : {username} (force)", "1;32"))
            cl = recharger_client()
            return cl, username
        except Exception as e:
            print(horloge(), color(f"❌ Echec restauration {username}: {e}", "1;31"))
            ajouter_a_blacklist(username, f"Restauration échec: {e}")
            return None, None
    ...
    try:
        ...
        return connexion_instagram_depuis_selected_user(force_restore=True)
...
async def effectuer_action(cl, action, id_cible, comment_text=None):
    try:
        ...
        if "login_required" in str(e).lower():
            ...
            cl2, _ = connexion_instagram_depuis_selected_user(force_restore=True)
            if cl2:
                try:
